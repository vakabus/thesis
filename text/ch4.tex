...
We did not confirm or disprove this hypothesis.
...
As discussed in \cref{chap:design}, the round trip time increase can be attributed to upcall processing, therefore we can estimate the cost of an upcall. The difference between the round trip time without an upcall and with an upcall is around $1$ \si{\milli\second}. More precisely:
\todo{The datapoint are not properly collected and also measured only on the virtualized hw. Repeat the experiment and replace the data with better. And maybe clean up the data?}
...
\begin{table}[h!]
    \begin{center}
        ...
        \begin{tabular}{r|S[table-format=5.2]S[table-format=5.2]S[table-format=-5.2]}
            ...
            \textbf{25th percentile} & 1500.0 & 733.0 & \xxx{??} \\
...
\paragraph{Possible fix implementation}
MAC spoofing protection for a single MAC address can be implemented using 48 rules, each checking for a single flipped bit in the Ethernet address compared to the allowed address. One additional rule would match the correct MAC address and resubmit the packet with a different \ident{recirc\_id} (see \cref{subsec:ovs-actions}). All other rules in the flow table would have to start with a non-default recirculation context. With multiple allowed Ethernet addresses, multiple recirculations would have to be chained.

However, to implement the protection as described, \ident{ovs-vswitchd} would have to change how the revalidator and the balancer threads work to prevent the eviction of these rules from the datapath. Alternatively, during an upcall, \ident{ovs-vswitchd} would have to check the already installed flow to find the missing rules. Either way, there is a performance penalty connected to all of these approaches. The kernel processing would be slower due to the increased number of flow table lookups.
...
ARP packets with varying hardware addresses behave the same as ordinary Ethernet packets with varying source hardware addresses because port security in OVN applies to them as well. The method of fixing this is also the same.
