...
\begin{itemize}
    \item Instead of a fixed flow eviction timeout, enforce the flow limit in \ident{ovs-vswitchd} by using a PID controller and a variable eviction timeout.
...
\begin{itemize}
    ...
    \item Diagnose and try to prevent a crash of \ident{ovs-vswitchd} when it is CPU limited
...
\subsection{Crashes when \ident{ovs-vswitchd} is CPU-limited}

In \cref{par:cpu-crashes}, we noticed that \ident{ovs-vswitchd} crashes under extreme stress when CPU-limited. We do not know what is the cause, so we can only suggest that more research should be done.
...
\xxx{nice to have, but let's worry about it later}
