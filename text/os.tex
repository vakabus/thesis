...
\begin{itemize}
    \item Edit the \ident{setup1-general.sh} script and change the \ident{configure\_systemd\_networkd} function to match your network environment.
...
\begin{enumerate}
    ...
    \item Copy the output of the previous step, add the \ident{-{}-cri-socket=unix:///var/run/cri-dockerd.sock} option to its end and run it on both \kb{2} and \kb{3}.

    \item Wait for the cluster to fully initialize, i.e. until the command \ident{kubectl get nodes} on \kb{1} shows that all 3 nodes are in the \ident{Ready} state.
    ...
    \item Now you can run experiments using the pods. Use \ident{kubectl exec -ti arch -- /bin/bash} to get a root shell in the \ident{arch} pod. Similarly for the \ident{victim} pod.
\end{enumerate}