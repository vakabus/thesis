//! In this experiment
//!
use std::{net::UdpSocket, thread, time::Duration, sync::{atomic::{AtomicBool, Ordering}, Arc}};

use chrono::Local;
use clap::Parser;

use crate::utils::{tcpdump::TcpDump, scapy::run_scapy_script};
...
pub fn run_experiment(_args: PacketFuzzingArgs) {
    let socket = UdpSocket::bind("0.0.0.0:9876").expect("failed to bind socket for receiving stats");
    socket.set_read_timeout(None).unwrap();
    info!("please run `analyzer log-flow-stats --log-ip {}` on Kubernetes node where the pod runs", socket.local_addr().unwrap());
    info!("waiting for statistics packet");

    // wait until we receive data
    let mut buf = vec![0u8; 8];
    let (_size, addr) = socket.recv_from(&mut buf).unwrap();
    info!("got a logging packet from {:?}, starting scapy script", addr);
    ...
    let _tcpdump = TcpDump::start_capture(&format!("packet_capture_{}.pcap", Local::now().to_rfc3339())).expect("tcpdump failed");
